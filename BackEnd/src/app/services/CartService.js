...
class CartService {
    ...
    addCouponCode = async (req, res) => {
        ...
        try {
            ...
            const couponItem = await couponRepository.getCouponByCode(code);
            if(!couponItem) throw new Error(`Code '${code}' not found!`);
            if(couponItem.usedCount >= couponItem.maxUsage) throw new Error('This coupon has reached its usage limit!');
